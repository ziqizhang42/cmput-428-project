...
As mentioned in \cite{newcombe2010}, when our current model is inaccurate, our image prediction algorithm can induce false discontinuities, in order to help alleviate this issue, we can apply smoothing. By smoothing the depth map in the reference frame, and then retriangulating, we can obtain a new mesh with fewer of these discontinuities. In said paper, there are 2 denoising functions which are mentioned that can be used here: A Gaussian Kernel with a support of 15 pixels or a TV-L$^1$ denoising algorithm. In practice, we decided to use the TV-L$^1$ algorithm, wherein we find a $\mathbf{D'}$ that minimizes the following function:

\begin{equation}
\mathbf{E_d} = \int_\Omega\gamma|\mathbf{D(x)}-\mathbf{D'(x)}|d\mathbf{x}+\int_\Omega g(\mathbf{x})|\nabla\mathbf{D'(x)}|d\mathbf{x}
\end{equation}

Our main reasoning for selecting the TV-L$^1$ algorithm over the, perhaps simpler, gaussian smoothing, is it's ability to preserve edges. The weight function $g$, is applied to the magnitude of the spatial gradient of the reference image $\nabla I_{ref}$ and maps it between 0 and 1 with the following formulation: $g(|\nabla I_{ref}|)=\exp(-\alpha|\nabla I_{ref}|^\beta)$. As can be seen, the weight will be large when the spatial gradient is small and vice-versa. Since the spatial gradient serves as a form of edge detection, as the value will only be large when there are sudden changes in the image intensity, this weight function ensures that sudden changes in depth will not be penalized if they occur across an edge. While \cite{newcombe2010}, specifies their parameters as $\alpha=10$ and $\beta = 1$, we found that these parameters were inaccurate, often inducing excessive changes in depth wherever there is even the slightest hint of an edge. This resulted in objects which were highly textured having many smaller discontinuities unlike the smoothness present in reality, as seen in figure ~\ref{denoise}. ****ADD SECTION DISCUSSING BEST VALUES FOR ALPHA AND BETA

\begin{figure}[h]
    \centering
    \includegraphics[width=\columnwidth]{denoise_example.png}
    \caption{Mesh after performing a denoising step, shows sudden depth changes on areas which are flat in reality}
    \label{denoise}
\end{figure}
